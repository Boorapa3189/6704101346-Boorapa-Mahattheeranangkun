...
const gotoApp = async (page: Page) => {
  // เปิดเว็บ Quasar dev server
  await page.goto('http://localhost:9000/#/'); // ← URL Quasar Dev ของคุณ
  // ตรวจสอบว่า root ของแอป Quasar โหลดขึ้นแล้ว
  await expect(page.locator('#q-app')).toBeVisible();
  // ตรวจสอบว่ามี form บนหน้า
...
// ระหว่าง animation ของ Quasar อาจมีหลาย message ซ้อนกัน ใช้ .last() เพื่อเลือกตัวล่าสุด
const messagesOf = (page: Page, label: string) =>
  fieldByLabel(page, label).locator('.q-field__messages').last();

// เลือก element โดยใช้ accessible locators ที่ Quasar render ออกมา
...
// ข้อความ error ที่รองรับหลายรูปแบบ (ตาม validation rule ของ Quasar)
